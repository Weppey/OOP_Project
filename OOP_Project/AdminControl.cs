    ...
    public partial class AdminControl : UserControl
    {
        ...
        private void SaveUserDetails(int userId)
        {
            string query = "UPDATE users SET username=@username, email=@email, age=@age, gender=@gender, " +
                           ...
                           "birthdate=@birthdate, email_verified=@emailVerified WHERE user_id=@userId";
            ...
            // Save the preferences (likely a comma-separated string, like the genres)
            cmd.Parameters.AddWithValue("@preferences", genre_clb.Text);  // Adjust if necessary

            // Use the selected security question from ComboBox
        ...
        {
            int userId = Convert.ToInt32(userID_tb.Text); // Assuming you have a hidden field for the userId
        ...
        private void verify_btn_Click(object sender, EventArgs e)
        {
            ...
            cmd.ExecuteNonQuery();
            MessageBox.Show("Email verification status updated!");
        }
        ...
        private void InsertNewUser()
        {
            ...
            string query = "INSERT INTO users (username, email, age, gender, preferences, security_question, security_answer, birthdate, email_verified) " +
                           "VALUES (@username, @email, @age, @gender, @preferences, @securityQuestion, @securityAnswer, @birthdate, @emailVerified)";
            ...
            cmd.Parameters.AddWithValue("@age", age_tb.Text);
            cmd.Parameters.AddWithValue("@gender", gender_cmb.SelectedItem.ToString());
            cmd.Parameters.AddWithValue("@preferences", string.Join(",", genre_clb.CheckedItems.Cast<string>().ToArray()));  // Assuming preferences are stored as comma-separated genres
            cmd.Parameters.AddWithValue("@securityQuestion", securityQuestion_cmb.SelectedItem.ToString());
            cmd.Parameters.AddWithValue("@securityAnswer", securityAnswer_tb.Text);
            cmd.Parameters.AddWithValue("@birthdate", Birthdate_dtp.Value.Date);
            cmd.Parameters.AddWithValue("@emailVerified", emailVerified_cb.Checked);
            ...
            // After insertion, reset the form for the next user to be added
            ResetUserForm();
            LoadUsers(); // Refresh the user list
        }
        ...
        private void UpdateUserDetails()
        {
            ...
            cmd.Parameters.AddWithValue("@age", age_tb.Text);
            cmd.Parameters.AddWithValue("@gender", gender_cmb.SelectedItem.ToString());
            cmd.Parameters.AddWithValue("@preferences", string.Join(",", userGenre_clb.CheckedItems.Cast<string>().ToArray()));  // Assuming preferences are stored as comma-separated genres
            cmd.Parameters.AddWithValue("@securityQuestion", securityQuestion_cmb.SelectedItem.ToString());
            cmd.Parameters.AddWithValue("@securityAnswer", securityAnswer_tb.Text);
            cmd.Parameters.AddWithValue("@birthdate", Birthdate_dtp.Value.Date);
            cmd.Parameters.AddWithValue("@emailVerified", emailVerified_cb.Checked);
            cmd.Parameters.AddWithValue("@userId", Convert.ToInt32(userID_tb.Text));  // Use the user ID from the form
        ...
        private void ResetUserForm()
        {
            ...
            isInsertingNewUser = true; // Set flag for inserting a new user
            insertUser_btn.Text = "Add User"; // Change button text to "Add User"
        }
    }
}
