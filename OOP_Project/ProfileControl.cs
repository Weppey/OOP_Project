...
namespace OOP_Project
{
    public partial class ProfileControl : UserControl
    {
                ...
                try
                {
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            // Populate checked list box preferences
                            preferences_clb.ClearSelected();
                            foreach (int i in Enumerable.Range(0, preferences_clb.Items.Count))
                                preferences_clb.SetItemChecked(i, false); // Uncheck all first

                            string preferences = reader["preferences"].ToString();
                            foreach (var item in preferences.Split(','))
                            {
                                int index = preferences_clb.Items.IndexOf(item.Trim());
                                if (index >= 0) // Check if item exists in the list
                            ...
                            {
                                avatar_pb.Image = null; // No avatar
                            }
                        }
                        else
                        {
        ...
        {
            // Replace with logic to get the current logged-in user's ID
        ...
        private void editProfile_btn_Click(object sender, EventArgs e)
        {
            if (onClicked == 0)
            {
                ...
                onClicked = 1;
                editProfile_btn.Values.Image = Properties.Resources.icons8_edit_user_28; // Switch to edit icon
            }
            else
            {
                // Disable editing
        ...
        private void changeAvatar_btn_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                {
                    avatar_pb.Image = Image.FromFile(ofd.FileName);
                }
            }
        }
    }
